...
For the national scale-up beyond New York City, imagery is instead drawn on demand from the USDA National Agriculture Imagery Program (NAIP) via the Microsoft Planetary Computer STAC catalog. NAIP covers the conterminous United States on state-specific $\sim$2--3 year flight cycles beginning around 2010, at a native ground sampling distance of 0.6\,m or 1\,m depending on state and year; each training crop is resampled to a fixed ground extent per pixel (crop size in meters divided by the model input size), so the effective resolution is constant by construction. Because flight years rarely coincide exactly with the requested panel year, each fetched crop carries its true acquisition year, and the training label is re-matched to the closest ACS panel year of that acquisition.
...
The survey data used in this study comes from the ACS 5-year direct survey estimates at the census tract level, using every annual vintage from 2011 through 2023, for all Core-Based Statistical Areas (metropolitan areas) with total population above 500{,}000. This dataset is chosen for its balance of spatial granularity and temporal coverage, allowing us to capture neighborhood-level economic dynamics while mitigating the high variance of single-year estimates. The default supervision target is a tract-level \emph{occupant-wealth index} ($W$) that capitalizes labor earnings over a tract-specific human-capital horizon, adds capitalized capital income, and adds per-capita net housing equity at a year-matched aggregate loan-to-value (our preferred variant, $W2$ at discount rate $\rho = 5\%$ and capital yield $r_k = 4.5\%$; see section \ref{sec:wealth}). The pipeline treats the indicator as a selectable parameter, so per-capita income and the other $W$ variants remain available as alternative labels for robustness.

To train a computer vision model to detect long-term capital accumulation, we must first isolate true structural neighborhood change from the high-variance sampling noise inherent in American Community Survey (ACS) 5-year estimates. If the network is trained on raw survey data, it risks optimizing for transient economic shocks or survey artifacts rather than physical infrastructure changes. We construct a robust, smoothed longitudinal panel ($t \in \{2011, \ldots, 2023\}$, aligned to the 2023 tract boundaries by maximum area overlap within each metro) using the following procedure.
...
To strip out city-wide macroeconomic inflation and aggregate growth trends, we convert the logged values into cross-sectional Z-scores relative to each metro's own distribution for each specific year (one normalization per CBSA per year, so scores are comparable within, never across, cities). Let $\mu_t$ and $\sigma_t$ be the metro-wide mean and standard deviation of $\ln Y_{i,t}$. The relative structural wealth score $Z_{i,t}$ and its corresponding relative standard error $\tilde{SE}_{i,t}$ are defined as:
\begin{equation}
...
\end{equation}
We establish a binary training gate at the 90\% confidence level ($p < 0.10$) on a \emph{single} first-versus-last comparison (2011 vs.\ 2023 for income; 2014 vs.\ 2023 for the wealth indexes, whose replicate-based standard errors are only published from 2014 onward). Let $I_{i}^{valid}$ be a boolean indicator equal to 1 if tract $i$ exhibits a change significant at that level over that span; tracts below the gate are treated as stable and anchor the temporal-invariance constraints. The gate is deliberately permissive---visual inspection of paired NAIP imagery shows physical shocks already registering at moderate significance levels---while the stricter $p<0.01$ flag and the full confidence-level brackets are retained separately for reporting. One such indicator is computed per candidate label (income and each $W$ variant), and the training pipeline uses the indicator matching the selected supervision target. Earlier drafts additionally filtered transient ``Yo-Yo'' shocks with a consecutive-pair macro gate; that gate has been removed in favor of the single long-difference test, whose endpoints are far enough apart that transient shocks do not register.
...
\subsection{Building Footprints}
\label{sec:buildings}

The national training universe is the Microsoft US Building Footprints dataset, a computer-vision-derived snapshot of roughly 130 million building polygons covering all fifty states. Because it carries no construction or demolition dates, we treat it as a \emph{static} universe: a building present in the snapshot is sampled in every panel year, which is sufficient for the ordinal ranking and stability objectives. Each building receives a deterministic identifier derived from its centroid (rounded to 0.1\,m in EPSG:5070), and only the centroid—never the polygon—enters the training path.

For the New York City evaluation exercises we retain the NYC Department of Information Technology and Telecommunications (DoITT) building footprint dataset. Each DoITT record is identified by a unique persistent identifier and includes a \texttt{CONSTRUCTION\_YEAR} field that we use to distinguish pre-existing buildings (constructed on or before 2009) from new construction added between 2010 and 2024—information the Microsoft snapshot lacks and the event study requires.

The building footprints serve two roles in the pipeline. First, each footprint's centroid defines the geographic anchor for aerial tile extraction (Section~\ref{sec:aerial}): the model observes a $\tau$-buffered image around the centroid, never the footprint polygon itself. Second, we aggregate new DoITT footprint area by census tract and year to construct the treatment indicator for the event study in Section~\ref{sec:results}. Specifically, for each tract we compute the ratio of cumulative new building footprint area (constructed after 2009) to the tract's pre-existing base footprint area. When this ratio crosses a threshold (1\,\%, 5\,\%, or 10\,\%), the tract is classified as treated in the Callaway--Sant'Anna difference-in-differences design \citep{csa}.
...
To assess out-of-sample performance across both spatial and temporal dimensions, the dataset is partitioned at the level of \emph{whole urban areas}, following the design of \citet{kachiyian2022}: every metropolitan area (CBSA) is assigned wholesale to the training, validation, or test set, so no city ever straddles two splits and no training tile can be spatially adjacent to a held-out one. Because held-out units are entire, disjoint metros, no buffer (``dead zone'') between splits is required.

\paragraph{Whole-city assignment, stratified by population}
CBSAs are first grouped into population brackets---\emph{mega} (the four most populous metros: New York, Los Angeles, Chicago, Houston), \emph{large} ($>$1M), \emph{medium} (750K--1M), and \emph{small} (500K--750K), with populations taken from the same ACS aggregation that defines the $>$500K estimation universe. Within each bracket, cities are assigned to train/validation/test targeting approximate shares of 50/20/30 measured in \emph{tract count} (the cross-sectional data share), via a deterministic greedy balancing rule with cities as atoms. The mega bracket is fixed at two training cities, one validation city, and one test city (seeded random assignment), so every size class---including the largest metros---is represented in every split. Validation cities form the held-out \texttt{val\_cities} set; test cities are never seen in any year.

\paragraph{Per-city temporal holdout}
Each \emph{training} city additionally holds out one imagery year (\texttt{val\_temporal}): the panel year closest to the midpoint of that state's NAIP coverage window, so the holdout is always an interpolation test rather than an extrapolation. Because NAIP flight years vary by state, the held-out year varies across cities rather than being a single global year; the imagery pipeline enforces the holdout on \emph{actual} flight years, rejecting training crops whose substituted acquisition year lands on the city's holdout year (and validation crops that land off it). This set evaluates generalization to an unobserved acquisition date---robustness to sun angle, vegetation phenology, and sensor vintage---within cities the model otherwise knows.

\paragraph{Validation sampling}
Because the within-city ranking metric is a per-(CBSA, year) Spearman correlation and the ACS labels are tract-level, its precision is set by how many \emph{tracts} each held-out city contributes, not by how many buildings or years. Each cross-sectional validation set---\texttt{val\_cities}, the two clustered within-city holdouts, and \texttt{val\_temporal}---is therefore sampled at one building per tract and one flight year per city (the year drawn independently across cities, so several NAIP vintages are represented), spending a fixed imagery budget on full tract coverage of every held-out city rather than on redundant repeat-year crops of a handful of tracts. The temporal-stability diagnostics below instead require the same building observed across years, so they are computed on a small separate pool (\texttt{val\_stability}) of multi-year buildings sampled per city.

\paragraph{Model selection and diagnostics}
The best checkpoint maximizes the mean \emph{within-city} Spearman rank correlation between predicted scores and ACS Z-score labels: the correlation is computed separately inside each (CBSA, year) cell---one observation per building, so buildings repeated across years never inflate the sample---with each cell weighted by its number of distinct tracts rather than buildings: labels are tract-level, so tract count is the cell's effective sample size, and building-count weights would let dense downtowns with many structures per tract dominate suburban cells of equal label information. The checkpoint score gives equal weight to a cross-sectional and a temporal component. The cross-sectional component pools the cells of \texttt{val\_cities} with those of two small spatially clustered within-city validation holdouts carved from New York and Chicago (quarantined from their surroundings by a dead-zone buffer; the closest training-time proxies for the two panels where the event-study analysis runs), so the two city holdouts enter at their natural tract weight rather than as separately averaged sets. The temporal component is the tract-weighted cell average of \texttt{val\_temporal}. This targets exactly the quantity the ordinal loss optimizes, and balances out-of-city generalization against out-of-year generalization. A pooled-across-cities Spearman is retained as a secondary diagnostic of cross-city rank alignment. Temporal behavior is monitored (not selected on) through two ordinal diagnostics on the multi-year \texttt{val\_stability} pool: the cross-year rank autocorrelation of stable buildings---compared against the same autocorrelation computed on the labels, which benchmarks genuine economic reshuffling---and the ratio of changed-to-stable within-building displacement, which verifies that predictions move where construction occurred. Validation diagnostics are additionally reported per population bracket and for the ten most populous metros individually---including the mean and standard deviation of the predicted indicator per bracket and the cross-city dispersion of per-city prediction means, which detect drift in the latent scale (each city's per-CBSA z-scored labels imply predictions distributed near $\mathcal{N}(0,1)$ everywhere)---so size-specific training pathologies are visible during training rather than post hoc.

Training uses AdamW with initial learning rate $10^{-4}$ and a ReduceLROnPlateau schedule (decay factor 0.8, patience 20 epochs, floor at $\mathrm{lr}/20$). Gradients are accumulated over 8 micro-batches before each optimizer step (physical batch size 8; effective batch size 64 cross-sectional images per update). The temporal change ranking loss $\mathcal{L}_{change}$ has been removed from the objective: change twins are still sampled into batches, but they carry no loss term and serve only the changed-twin validation metrics (Section~\ref{sec:model}).
...
\subsubsection{\textbf{Feature Extraction: ScaleMAE with Low-Rank Adaptation}}
...
\subsubsection{\textbf{Late Fusion Score Head}}

A Late Fusion Head projects the pooled image embedding---concatenated with a geographic covariate vector---onto a single scalar score. The architecture consists of: (i) A linear compression layer that maps the 1024-dimensional image embedding to 64 dimensions, followed by GELU activation, layer normalization, and 40\% dropout; (ii) A final linear layer that maps the concatenation of the 64-dimensional compressed image features and the metadata vector $\mathbf{m}_{b,i,t}$ (\textbf{currently a single covariate: distance-to-city-center}) to the scalar output.
...
This scalar $\hat{R}_{b,i,t}$ represents the building's position on the ordinal poor-to-rich continuum. Within a census tract $i$, buildings share the same underlying socioeconomic context, so in expectation, $E_i \hat{R}_{b,i,t} = \hat{R}_{i,t}$ for all $b$. Deviations across buildings within the same tract are treated as measurement error during training. Because the model is never fed more than a few buildings per tract per epoch, it is regularized against overfitting to the visual idiosyncrasies of individual structures and is instead forced to learn the structural hierarchy of wealth at the neighborhood level.

\subsubsection{\textbf{Training Batch Construction and Sampling Strategy}}

The training pipeline constructs stochastically stratified \textbf{hybrid batches}, each designed to maximize pairwise supervision density from a single unified forward pass through the backbone. Figure \ref{fig:batch} illustrates the batch construction process.

\begin{figure*}
    \centering
    \caption{Hybrid batch construction example.}
    \label{fig:batch}
    \includegraphics[width=\linewidth]{Figures/Batch example (Chat GPT).png}
    % \caption{Hybrid batch construction: a single anchor year $t$ is randomly selected, and $B_{cs}$ distinct buildings from different census tracts are drawn to form the cross-sectional core $\mathcal{B}_{cs}$. For a fraction $\rho_{temp}$ of these buildings, the sampler queries the longitudinal panel for stable temporal twins in a different year $t'$, which are appended to the batch as $\mathcal{B}_{temp}$. The complete batch $\mathcal{B} = \mathcal{B}_{cs} \cup \mathcal{B}_{temp}$ is processed in a single forward pass, with decoupled losses applied to the respective subsets.}
\end{figure*}


\paragraph{The Cross-Sectional Core ($\mathcal{B}_{cs}$)}
For every batch, a single anchor year $t$ is randomly selected and draws $B_{cs}$ (in this iteraturion, X) distinct buildings from different census tracts observed in that year. This cross-sectional core is the primary source of ordinal ranking supervision: every unique pair of buildings from different census tracts provides a valid directional constraint.

\paragraph{The Temporal Auxiliary Set ($\mathcal{B}_{temp}$)}
The cross-sectional core $\mathcal{B}_{cs}$ is drawn from a single (year, metro) pool—per-metro relative scores are only comparable within a CBSA, so cross-metro pairs never enter the ranking loss and every batch contains a single city—with pools sampled proportionally to their size. For a fraction of the buildings in $\mathcal{B}_{cs}$ (controlled by the hyperparameter $\rho_{temp}$), the sampler queries the longitudinal panel for observations of the same building in a different year $t' \neq t$. The temporal budget is split evenly between \emph{stable} and \emph{change} buildings according to the structural change indicator $I_{k}^{valid}$ (see section \ref{sec:acs}), backfilling from the other side when one is scarce; the loss applies the stability penalty only to stable twins, while change twins carry no loss term at all—they are sampled solely to feed the changed-twin displacement and directional-accuracy validation metrics—so the comparability constraint is enforced in the loss masks rather than by discarding change twins at sampling time. Because labels are tract-level quantities, the sampler draws census \emph{tracts} (one building per drawn tract) rather than raw buildings, which would over-represent low-density areas with many small structures. Tract draws are further importance-weighted by an exponential moving average of each tract's per-sample ranking gradient $|\lambda_i|$ \citep{burges2010ranknet}---the variance-optimal sampling criterion of \citet{katharopoulos2018notall}---mixed with a uniform component and capped so that hard-example mining \citep{shrivastava2016training} cannot concentrate on statistically unreliable or irreducibly hard tracts \citep{mindermann2022prioritized}.
...
A single forward pass encodes the entire hybrid batch $\mathcal{B}$ into a vector of scalar scores $\hat{R}$. The total loss is the decoupled sum of two objectives computed over valid subsets of these scores, plus a variance regularizer that prevents score collapse.

\paragraph{RankNet Loss for Cross-Sectional Ranking ($\mathcal{L}_{cross}$)}
...
The cross-sectional loss uses a smooth logistic (RankNet) surrogate:
\begin{equation}
...
% \subsubsection{\textbf{Curriculum learning: easy-to-hard pair filtering.}}
...
\subsubsection{\textbf{Out-of-Sample Generalizability and Quantile Mapping}}
...
\subsubsection*{\textbf{Summary of Core Architectural Hyperparameters (Runtime Values)}}
\begin{itemize}
    ...
    \item \textbf{$\rho_{temp}$ (Temporal Fraction):} Fraction of the batch reserved for temporal auxiliary images (budget split 50/50 between stable and change twins). Value: 0.40.
    % \item \textbf{$m_{base}$ (Fixed Temperature):} The temperature $T = m_{base}$ used in the logistic ranking surrogate. Value: 1.0.
    \item \textbf{$\lambda_s$ (Stable Temporal Weight):} Weighting of the temporal stability L2 penalty. Value: 0.3. (The change-ranking term and its weight $\lambda_c$ were removed from the objective; change twins feed validation metrics only.)
\end{itemize}
...
Before presenting the event-time plots, it is necessary to formally define the econometric object this exercise recovers. In a standard causal inference setting, a difference-in-differences design estimates the causal effect of a treatment on a realized economic outcome. In our framework, however, the outcome variable is the model's predicted ordinal score $\hat{R}_{i,t}$ aggregated to the tract level. 

Viewing the neural network as a non-parametric estimator of a conditional expectation function, the model recovers the expected neighborhood wealth $W_{i,t}$ given the observed physical built environment (the aerial imagery) $X_{i,t}$. That is, $\hat{R}_{i,t} \approx \mathbb{E}[W_{i,t} \mid X_{i,t}]$. 

Following \citet{callaway2021difference}, let $g$ denote the first year a census tract crosses the new construction threshold and let $\hat{R}_{i,t}(g)$ denote the potential predicted score for tract $i$ at time $t$ if treated in year $g$, with $\hat{R}_{i,t}(\infty)$ denoting the potential score if never treated. The theoretical Group-Time Average Treatment Effect on the Treated ($ATT$) for our predicted scores is defined as:
\begin{equation}
    ATT_{model}(g,t) = \mathbb{E}\left[ \hat{R}_{i,t}(g) - \hat{R}_{i,t}(\infty) \mid G_i = g \right]
\end{equation}

By substituting the model's conditional expectation into this estimand, the exact statistical object we are evaluating becomes clear:
\begin{equation}
    ATT_{model}(g,t) = \mathbb{E} \left[ \mathbb{E}[W_{i,t}(g) \mid X_{i,t}(g)] - \mathbb{E}[W_{i,t}(\infty) \mid X_{i,t}(\infty)] \;\middle|\; G_i = g \right]
\end{equation}

Crucially, because our treatment condition---new building construction---mechanically alters the physical built environment, the visual features under treatment differ from those under control ($X_{i,t}(g) \neq X_{i,t}(\infty)$). 

Therefore, this event study is \textbf{not} estimating the causal effect of new construction on true economic wealth. Rather, it measures the \textit{responsiveness of the model to structural shocks}. It tests whether the model successfully updates its capitalized visual wealth signal when the underlying physical capital stock changes. In the context of satellite-based machine learning---where models frequently suffer from temporal label drift and struggle to identify longitudinal change without retraining---this design serves as a rigorous internal validation tool. A strictly positive $ATT_{model}$ proves that the ordinal ranking architecture overcomes the temporal drift problem by actively shifting latent representations in response to new physical capital.

To formally test this responsiveness, we implement a staggered difference-in-differences at the census-tract level. The treatment is defined as the first year in which a tract's cumulative new building footprint area (buildings constructed 2010--2024) crosses a given percentage of its pre-2010 base footprint area. We consider three thresholds: 1\%, 5\%, and 10\%, capturing mild, moderate, and intensive construction activity. The outcome variable is the tract-level average model prediction (i.e., the mean ordinal score). Never-treated tracts serve as the control group.
...
\subsection{Qualitative Case Study: Hudson Yards}

To provide a concrete, spatially-anchored illustration of the model's capabilities, Figure~\ref{fig:hudson_yards} traces the evolution of the Hudson Yards neighborhood on the west side of Manhattan from 2010 to 2024. The left column displays the raw aerial imagery, highlighting the transformation of an open rail yard and industrial space into a high-density luxury mega-development. The middle column isolates the building footprints, using official cadastral data to distinguish pre-existing baseline structures (grey) from new construction built after 2009 (red). 

The right column maps the model's building-level predicted scores. In the earlier years (2010--2014), the model correctly assigns relatively modest to lower structural wealth scores to the area's pre-existing low-density buildings. However, as the massive new towers are constructed and appear in the imagery (2016 onwards), the model accurately traces the accumulation of capital, assigning them high relative structural wealth scores. 

Crucially, the model strictly isolates this new wealth to the specific footprints of the new mega-structures, while maintaining stable, lower predictions for the unchanged pre-existing buildings situated further blocks away. Although the census tracts containing Hudson Yards are included in the train set, this sequence demonstrates the sub-tract, building-level power of the model. It seamlessly translates localized visual evidence of structural transformation into dynamic, high-resolution wealth trajectories, capturing steep micro-spatial inequalities and rapid gentrification that static, tract-averaged census data fundamentally obscures.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{Figures/D_hudson_yards_grid.pdf} % Update filename if needed
    \caption{Hudson Yards neighborhood evolution (2010--2024). Left: Aerial imagery. Middle: Building footprints. Right: Model predictions at the building level.}
    \label{fig:hudson_yards}
\end{figure}
