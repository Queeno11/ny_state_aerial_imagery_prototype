...
The best checkpoint maximizes the mean Spearman rank correlation between predicted scores and ACS Z-score labels across the two validation sets, so model selection balances out-of-city generalization (\texttt{val\_cities}) against out-of-year generalization (\texttt{val\_temporal}). Validation diagnostics are additionally reported per population bracket and for the ten most populous metros individually---including the mean and standard deviation of the predicted indicator per bracket, which detects cross-city drift in the latent scale (each city's per-CBSA z-scored labels imply predictions distributed near $\mathcal{N}(0,1)$ everywhere)---so size-specific training pathologies are visible during training rather than post hoc.
