...
def crop_dataset_by_polygon(ds, poly):
    ...
    area_selected = ds.sel(x=slice(x_min, x_max), y=slice(y_max, y_min)).band
...
def ds_plot_example(ds, poly, ax):
    ...
    image = geo_utils.process_image(image, resizing_size=image.shape[1])[:, :, :3]
...
def generate_grid(
    ...
    grid_preds_folder = rf"{PROCESSED_DATA_DIR}/gridded_predictions/{savename}"
...
def plot_example(grid_preds, bbox, modelname, datasets, extents, img_savename):
    ...
    ds_names = geo_utils.get_datasets_for_polygon(poly, extents)

    icpag = build_dataset.load_income_dataset(variable="ln_pred_inc_mean", trim=False)
    ...
    savepath = f"{RESULTS_DIR}/{modelname}"
    os.makedirs(savepath, exist_ok=True)
    plt.savefig(
        f"{RESULTS_DIR}/{modelname}/{modelname}_{img_savename}.png",
        bbox_inches="tight",
        dpi=300,
    )
    print(
...
def plot_time_evolution(
    ...
    # Carga datasets a memoria
    ds_names_pre = geo_utils.get_datasets_for_polygon(poly, extents_pre)
    ds_names_post = geo_utils.get_datasets_for_polygon(poly, extents_post)
    ...
    os.makedirs(savepath, exist_ok=True)
    plt.savefig(
        f"{RESULTS_DIR}/{modelname}/{modelname}_{img_savename}_evolution.png",
        bbox_inches="tight",
        dpi=300,
    )
    print(
...
def plot_grid(grid_preds, modelname, year=2013):
    ...
    os.makedirs(savepath, exist_ok=True)
    plt.savefig(
        f"{RESULTS_DIR}/{modelname}/{modelname}_amba_{year}.png",
        bbox_inches="tight",
        dpi=300,
    )
    print(
        "Se creó la imagen: ", f"{RESULTS_DIR}/{modelname}/{modelname}_amba_{year}.png"
    )
...
if __name__ == "__main__":
    ...
    # path_repo = PROJECT_ROOT
    # extra = "_aug"
    # savename = f"{model_name}_size{image_size}_tiles{tiles}_sample{sample_size}{extra}"
    # savename = "effnet_v2S_lr0.0001_size128_stack1-4"
    # year = 2018

    # datasets, extents, df = main.open_datasets(
    #     sat_data="aerial", years=[2010, 2012, 2014, 2016, 2018, 2020, 2022, 2024]
    # )
