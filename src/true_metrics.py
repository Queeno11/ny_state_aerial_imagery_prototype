...
def get_gridded_predictions_for_grid(
    ...
    def remove_sea_from_grid(grid):
        ...
        is_relevant = grid.set_crs(epsg=6539, allow_override=True).within(amba_costa)
    ...
    def load_roi_coast_data(grid):
        ...
        # Load país
        pais = gpd.read_file(rf"{EXTERNAL_DATA_DIR}/Limites Argentina/pais.shp")
        # Get grid exterior
        exterior = Polygon(
            [
    ...
    def procesa_grilla(grid, icpag, extents):
        ...
        grid = grid.set_crs(epsg=6539, allow_override=True)
    ...
    def get_grid_data(i):
        ...
        # Generate the image
        image = geo_utils.stacked_image_from_census_tract(
    ...
    if os.path.exists(rf"{PROCESSED_DATA_DIR}/grid_datasets/grid_{year}_proc.parquet"):
        grid = gpd.read_parquet(
            rf"{PROCESSED_DATA_DIR}/grid_datasets/grid_{year}_proc.parquet"
        )
        # grid = grid.cx[-58.58:-58.52, -34.49:-34.52].reset_index()
    else:
        if size < 128:  # Working with landsat
            grid = gpd.read_parquet(
                rf"{EXTERNAL_DATA_DIR}/Grillas/grid_size512_tiles1.parquet"
            )
        else:
            grid = gpd.read_parquet(
                rf"{EXTERNAL_DATA_DIR}/Grillas/grid_size{size}_tiles1.parquet"
            )
        grid = procesa_grilla(grid, icpag, extents)
        grid.to_parquet(rf"{PROCESSED_DATA_DIR}/grid_datasets/grid_{year}_proc.parquet")
...
def compute_custom_loss_all_epochs(
    ...
    if subset == "test":
        df = gpd.read_feather(
            rf"{PROCESSED_DATA_DIR}/test_datasets/{savename}_test_dataframe.feather"
        )
    elif subset == "val":
...
def compute_custom_loss_for_epoch(
    ...
    if subset == "test":
        ...
        df = df_not_test.sample(frac=0.066667, random_state=200)
        df = df.reset_index()
        df.to_csv(rf"{PROCESSED_DATA_DIR}/val_datasets/{savename}_val_dataframe.feather")
    print("Data loaded!")
